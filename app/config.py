...
class Settings(BaseSettings):
    ...
    embedding_model: str = Field(
        ...
        description="Embedding 模型（DashScope text-embedding-v3 输出 1024 维）",
    )

    database_url: str = Field(
        default="mysql+aiomysql://root:root@localhost:3306/agent_db",
        description="SQLAlchemy 异步数据库 URL（推荐 mysql+aiomysql）",
    )

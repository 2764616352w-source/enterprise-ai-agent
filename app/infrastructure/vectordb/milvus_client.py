...
class MilvusManager:
    ...
    async def create_collection(self, name: str, dim: int) -> None:
        """创建向量集合（含 id/embedding/text/doc_id 字段）；若已存在则跳过。"""
    ...
    async def insert(
        ...
        metadata 可含 ``text``、``doc_id`` 等字段，将与 id 一同写入集合。
        """
        await self._ensure_connection()
        ...
        for i, m in enumerate(metadata):
            ...
            doc_ids.append(str(m.get("doc_id", "")))

        def _insert() -> None:
            col = Collection(collection, using=self._alias)
            # 与 create_collection 字段顺序保持一致：
            # id, embedding, text, doc_id
            col.insert([ids, vectors, texts, doc_ids])
            col.flush()
